...
function Pedidos() {
    ...
    <table className="pedidos-table">
        ...
        <tbody>
            {data.map(({id_reserva, tipo_servicio, hora_inicio, tecnico_asociado, remitente, destinatario, origen, destino, observaciones}) => (
                <tr key={id_reserva}>
                    <td>{id_reserva}</td>
                    <td>{tipo_servicio}</td>
                    ...
                    <td className="options-cont">
                        { idOptions === id_reserva ? options(setIdOptions) : 

                        <svg xmlns="http://www.w3.org/2000/svg" fill="white" height="2em" viewBox="0 0 24 24" className="optionIcon" onClick={() => setIdOptions(id_reserva)}>
